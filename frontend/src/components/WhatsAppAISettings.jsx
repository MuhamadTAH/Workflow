...
const WhatsAppAISettings = ({ isVisible, onClose }) => {
  ...
  // System Prompt State
  const [systemPrompt, setSystemPrompt] = useState('You are a helpful and friendly AI assistant for WhatsApp messages. Respond to users in a professional yet warm manner.');
  ...
  useEffect(() => {
    if (isVisible) {
      console.log('🔄 WhatsApp AI Settings modal opened - loading current settings...');
      loadConfig();
      checkClaudeStatus();
      loadSystemPrompt();
  ...
  const testAI = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/whatsapp-receiver/ai-test`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
  ...
  const handleClaudeConnect = async () => {
    ...
    try {
      ...
      if (response.ok && result.success) {
        setClaudeStatus('✅ Claude AI connected! AI is now automatically enabled for WhatsApp');
        ...
        await fetch(`${API_BASE_URL}/api/whatsapp-receiver/ai-config`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${localStorage.getItem('token')}`
  ...
  const resetToDefaultPrompt = () => {
    setSystemPrompt('You are a helpful and friendly AI assistant for WhatsApp messages. Respond to users in a professional yet warm manner.');
  };

  if (!isVisible) return null;
  
  console.log('🚀 WhatsAppAISettings is rendering with new design!');
          ...
          <div style={{ display: 'flex', gap: '1rem' }}>
            {!isClaudeConnected ? (
              <button
                ...
                style={{ 
                  flex: '1',
                  backgroundColor: isConnectingClaude || !claudeApiKey.trim() ? '#9ca3af' : '#25D366', 
                  color: 'white', 
        ...
        <div style={{ marginBottom: '2rem' }}>
          ...
          <div style={{ marginBottom: '1rem', opacity: isClaudeConnected ? 1 : 0.6 }}>
            ...
            <textarea
              ...
              placeholder="Enter how Claude should behave when responding to WhatsApp messages..."
              rows={4}
              style={{
                width: '100%',
                padding: '0.75rem',
