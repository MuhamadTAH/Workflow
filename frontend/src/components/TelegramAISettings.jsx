...
const TelegramAISettings = ({ isVisible, onClose }) => {
  ...
  // System Prompt State
  const [systemPrompt, setSystemPrompt] = useState('You are a helpful and friendly AI assistant for Telegram messages. Respond to users in a professional yet warm manner.');
  ...
  useEffect(() => {
    if (isVisible) {
      console.log('🔄 Telegram AI Settings modal opened - loading current settings...');
      loadConfig();
      checkClaudeStatus();
      loadSystemPrompt();
  ...
  const checkClaudeStatus = async () => {
    try {
      ...
      if (isConnected) {
        setClaudeStatus('✅ Claude API ready for Telegram integration');
      }
    } catch (error) {
  ...
  const handleClaudeConnect = async () => {
    ...
    try {
      ...
      if (response.ok && result.success) {
        setClaudeStatus('✅ Claude AI connected! AI is now automatically enabled for Telegram');
  ...
  const resetToDefaultPrompt = () => {
    setSystemPrompt('You are a helpful and friendly AI assistant for Telegram messages. Respond to users in a professional yet warm manner.');
  };

  if (!isVisible) return null;
  
  console.log('🚀 TelegramAISettings is rendering with new design!');
      ...
      <div style={{
          ...
          <div style={{ display: 'flex', gap: '1rem' }}>
            {!isClaudeConnected ? (
              <button
                ...
                style={{ 
                  flex: '1',
                  backgroundColor: isConnectingClaude || !claudeApiKey.trim() ? '#9ca3af' : '#0088cc', 
                  color: 'white', 
        ...
        <div style={{ marginBottom: '2rem' }}>
          ...
          <div style={{ marginBottom: '1rem', opacity: isClaudeConnected ? 1 : 0.6 }}>
            ...
            <textarea
              ...
              placeholder="Enter how Claude should behave when responding to Telegram messages..."
              rows={4}
              style={{
                width: '100%',
                padding: '0.75rem',
        ...
        <div style={{ 
          ...
          <button
            ...
            disabled={isLoading}
            style={{
              backgroundColor: isLoading ? '#9ca3af' : '#0088cc',
              color: 'white',
              border: 'none',
              borderRadius: '6px',
              padding: '0.75rem 1.5rem',
