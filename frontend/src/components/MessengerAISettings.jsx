...
const MessengerAISettings = ({ isVisible, onClose }) => {
  // AI Configuration State
  const [config, setConfig] = useState({
    enabled: true, // Enable AI by default
    systemPrompt: 'You are an AI assistant for Facebook Messenger. Respond naturally and helpfully.',
    knowledgeBase: '',
    autoReply: true,
  ...
  // System Prompt State
  const [systemPrompt, setSystemPrompt] = useState('You are an AI assistant for Facebook Messenger. Respond naturally and helpfully.');
  ...
  useEffect(() => {
    if (isVisible) {
      console.log('🔄 Messenger AI Settings modal opened - loading current settings...');
      loadConfig();
      checkClaudeStatus();
      loadSystemPrompt();
  ...
  const checkClaudeStatus = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/messenger-ai/validate`);
      const data = await response.json();
      ...
      if (isConnected) {
        setClaudeStatus('✅ Claude API ready for Messenger integration');
      }
    } catch (error) {
  ...
  const handleClaudeConnect = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/messenger-ai/connect`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      if (response.ok && result.success) {
        setClaudeStatus('✅ Claude AI connected! AI is now automatically enabled for Messenger');
        ...
        await fetch(`${API_BASE_URL}/api/messenger-ai/config`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
  ...
  const handleClaudeDisconnect = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/messenger-ai/disconnect`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        }
  ...
  const handleSystemPromptSave = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/messenger-ai/system-prompt`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  const loadKnowledgeBaseInfo = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/messenger-ai/knowledge-info`);
      const result = await response.json();
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/messenger-ai/upload-knowledge`, {
        method: 'POST',
        body: formData
      });
  ...
  const resetToDefaultPrompt = () => {
    setSystemPrompt('You are an AI assistant for Facebook Messenger. Respond naturally and helpfully.');
  };

  if (!isVisible) return null;
  
  console.log('🚀 MessengerAISettings is rendering with new design!');
          ...
          <div style={{ display: 'flex', gap: '1rem' }}>
            {!isClaudeConnected ? (
              <button
                ...
                style={{ 
                  flex: '1',
                  backgroundColor: isConnectingClaude || !claudeApiKey.trim() ? '#9ca3af' : '#0084ff', 
                  color: 'white', 
        ...
        <div style={{ marginBottom: '2rem' }}>
          ...
          <div style={{ marginBottom: '1rem', opacity: isClaudeConnected ? 1 : 0.6 }}>
            ...
            <textarea
              ...
              placeholder="Enter how Claude should behave when responding to Messenger messages..."
              rows={4}
              style={{
                width: '100%',
                padding: '0.75rem',
...
export default MessengerAISettings;