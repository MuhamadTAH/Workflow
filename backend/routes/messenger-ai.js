...
// Messenger AI Configuration Storage
let aiConfig = {
...
function handleMessageBatch(senderId, messageText, generateAIReply, sendMessengerReply) {
  ...
  batch.timeout = setTimeout(async () => {
    try {
      ...
      if (aiReply) {
        // Send the AI reply
        await sendMessengerReply(senderId, aiReply, true);
...
router.get('/messenger-ai/config', (req, res) => {
  logger.info('📋 Messenger AI config requested');
...
// Test AI response
router.post('/messenger-ai/test', async (req, res) => {
...
router.get('/messenger-ai/validate', async (req, res) => {
...
router.post('/messenger-ai/connect', async (req, res) => {
  ...
  logger.info('🔗 Connecting to Claude AI for Messenger');
  ...
  try {
    ...
    if (result.valid) {
      ...
      isConnected = true;
      logger.info('✅ Claude AI connected for Messenger integration');
...
router.post('/messenger-ai/disconnect', async (req, res) => {
  logger.info('🔌 Disconnecting Claude AI for Messenger');
...
// Get system prompt
router.get('/messenger-ai/system-prompt', (req, res) => {
  logger.info('📋 Messenger AI system prompt requested');
...
router.post('/messenger-ai/system-prompt', (req, res) => {
  ...
  logger.info('💾 Updating Messenger AI system prompt');
...
router.get('/messenger-ai/knowledge-info', (req, res) => {
  logger.info('📚 Messenger AI knowledge base info requested');
...
router.post('/messenger-ai/upload-knowledge', upload.single('pdf'), async (req, res) => {
